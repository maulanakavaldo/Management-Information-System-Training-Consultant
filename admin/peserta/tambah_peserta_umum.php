...
if (isset($_POST['simpan'])) {
  ...
  $lihat = mysqli_query($CON, "SELECT * from tb_pendaftaran where no_ktp = '$no_ktp' or username = '$username'");
  ...
  if ($cek >= 1) {
    ...
    $update_kuota = mysqli_query($CON, "UPDATE tb_pelatihan SET kuota = kuota-1 WHERE id_pelatihan='$nama_pelatihan'") or die(mysqli_error($CON));
      ...
      <td width="66%">
        <div class="col-sm-10">
          <select class="form-control" name="nama_pelatihan">
            <option>Pilih*</option>
            <?php
            $s_pelatihan = mysqli_query($CON, "SELECT * FROM tb_jenispelatihan left join tb_pelatihan on tb_jenispelatihan.id_jenispelatihan=tb_pelatihan.id_jenispelatihan WHERE kuota > 0 ") or die(mysqli_error($CON));
