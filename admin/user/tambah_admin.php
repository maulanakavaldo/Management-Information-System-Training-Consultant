...
if (isset($_POST['simpan'])) {
  if (@$_FILES['foto']['name']) {
    ...
    $target     =  '../foto/user/';
    $nama_foto  = $id_admin . '_' . $_FILES["foto"]["name"];

    $pindah     = move_uploaded_file($sumber, $target . $nama_foto);

    $simpan = mysqli_query($CON, "INSERT INTO tb_admin VALUES('$id_admin','$nama','$jekel','$alamat','$no_hp', '$username', '$password', '$nama_foto', '$status')");

    // $simpan2 = mysqli_query($CON, "INSERT INTO tb_hubung VALUES ('$id_admin','$jns_magang')") or die(mysqli_error($CON));
...
$query = "select max(id_admin) from tb_admin";
...
if ($kode) {
  ...
  $kodenya = $kodenya + 1;
  $hasilkode = "A" . str_pad($kodenya, 4, "0", STR_PAD_LEFT);
} else {
  $hasilkode = "A0001";
}
?>
              ...
              <form class="form-horizontal" method="post" enctype="multipart/form-data">
                <div class="box-body">
                  <div class="form-group">
                    ...
                    <div class="col-sm-6">
                      <input type="text" class="form-control" name="id_admin" value="<?php echo $hasilkode ?>" readonly>
                    </div>
                  </div>
                  <div class="form-group">
